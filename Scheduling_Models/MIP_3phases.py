...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug 27 09:01:57 2021

add new constr min number of patients starting on same day same machine
...
CT_hours=pd.read_excel("CThours_endOfWarmup.xlsx")
LINAC_hours=pd.read_excel("Linachours_endOfWarmUp.xlsx")
df_partial = pd.read_excel("partialBooked_endOfWarmup.xlsx")


# read sample input instance
instance_all=pd.read_excel("ins_2018_2020_part2_QuantileRules.xlsx")
...
# output record file
output_file = "record_201912_202003_80p_minSameDaySameLinac.xlsx"
...
df_record = pd.DataFrame(columns=['MRN','Category','Intent', 'CreatedDate','CTWait','ModelWaitTime','CTUnit','LinacUnit','TxDuration','numFracs'])
...
def modelConstrs(model):
    ...
    model.addConstrs(((z[j]==1)>>(W[j]>=tw[j]+1) for j in J), name='wait_time_z_var')
    ...
    model.addConstrs((d2[j] <= 20 for j in J), name = 'book_first_20_days') # ...new constraint
...
for date in all_dates:
    ...
    #model.setObjective(quicksum(z[j]+pr[j]*O[j] for j in J), GRB.MINIMIZE)
    model.setObjective(quicksum(z[j] for j in J), GRB.MINIMIZE) 
    model = modelConstrs(model)
    model.optimize()
    
    newConstr = model.objVal # end of Phase 1
    sumZ.append(newConstr)
    
    
    ### CreatedGurubi model Phase 2..............................................................
    with gp.Env(empty=True) as env:
        env.setParam('LogToConsole', 0)
        env.setParam('OutputFlag', 0)
        env.start()
        with gp.Model(env=env) as model:
            model = gp.Model("RT_Patient_Scheduling_2nd")
            model.setParam('TimeLimit', 5*60)
    # create a list of tuples containing every iteration of j,d,c
    # the tuples are the same as phase 1
    ...
    b_tuple = [(d,l) for d in H for l in LINACs]
    b = model.addVars(b_tuple, vtype = GRB.INTEGER, name = "b_dl")
    
    model.setObjectiveN(quicksum(O[j] for j in J), 0, 1)
    model.setObjectiveN(quicksum(b[d,l] for d in H for l in LINACs), 1, 0)
    
    model.ModelSense = GRB.MINIMIZE
   
    
    model.addConstr(quicksum(z[j] for j in J) <= newConstr) ### new phase 2 constraint  
    # define b variables: if b=1, same machine same day is booked for multiple patients; 
    # if b=0, same day same machine has only one patients
    model.addConstrs(((b[d,l]==0)>>(quicksum(y[j,d,l,0] for j in J)<=1) for d in H for l in LINACs), name='same_day_same_linac')
    model.addConstrs(((b[d,l]==1)>>(quicksum(y[j,d,l,0] for j in J)>=2) for d in H for l in LINACs), name='sdsl')
    
    model = modelConstrs(model) # other common constraints
    
    model.optimize() # end of phase 2
    ...
    for i in x_jdc:
        ...
        new = current - time_c    
        CT_hours.at[CT_hours['day']==d, ct] = new 
    ...
    df2 = pd.DataFrame([[20, 190, 147, 83],[21,1000,1000,1000]], columns=['day',2,3,4])
                        #20, 170, 235, 137
    ...
    # add two new rows: one for a new day, the other for the arbituary day
    day20 = [375,318,349,320,478,543,409,436,464,502,415,570,0,420,329,450]
            #275,318,349,220,478,543,409,436,464,502,415,570,0,420,329,450
    ...
    #linac_hours_updated = linac_hours_updated.where(linac_hours_updated < 0, 0) # replace negative value with 0
    ...
    for i in y_jdls:
        ...
        if s == 0: # record the first treatment, if first tx is booked the patient is booked
            list_record.append([MRN, cate, intent, createdDate, ctWait, totalWait, ct, linac, duration, numFracs])
    df_record = df_record.append(pd.DataFrame(list_record, columns=df_record.columns))
    
    # Record patients who are partially booked (have appt on arbituary day 21)............................................................
    # and book the remaining appointment on consecutive days
    list_partial = []
    for i in y_jdls:
        p=i[0]
        d=i[1]
    ...
    for index, row in df_partial.iterrows():
        ...
        MRN = row['MRN']
        TxDur = row['TxDur']
        linac_hours_updated.loc[20, unit] -= TxDur - apptDiff # this row is day 20
        #linac_hours_updated = linac_hours_updated.where(linac_hours_updated < 0, 0)
        linac_hours_updated[linac_hours_updated < 0] = 0
    ...
    if date >= np.datetime64('2019-12-01T00:00:00.000000000'):
        z_obj += newConstr
        objVal += model.objVal
...
#df_partial.to_excel('partialBooked_endOfWarmup_2.xlsx')
#LINAC_hours.to_excel('Linachours_endOfWarmUp_2.xlsx')
#CT_hours.to_excel('CThours_endOfWarmup_2.xlsx')
#machine_hours_booked.to_excel('util_202003_202006_80p.xlsx')
...
plt.title('wait time distribution of 80p 2020 new')
plt.xticks(fontsize=10)
plt.yticks(fontsize=10)

plt.show()

# compute total over target wait time




print('mean: ', df_record['ModelWaitTime'].mean())
print('std: ', df_record['ModelWaitTime'].std())
print('wait time quantiles:\n', df_record['ModelWaitTime'].quantile([.1, .5, .75]))
print('number of exceed: ', len(df_record[df_record['ModelWaitTime']>10]))
print('urgent: ', df_record[df_record['Category']=='Urgent 2']['ModelWaitTime'].mean())
print('urgent & palliative: ', df_record[(df_record['Category']=='Urgent 2')&
                                           (df_record['Intent']=='Palliative')]['ModelWaitTime'].mean())
